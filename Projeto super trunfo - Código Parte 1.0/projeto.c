...
void inserirCarta(Cartas **cartas, int *quantidadeCartas)
{
    ...
    *cartas = novasCartas;
    ...
    for (int i = 0; i < *quantidadeCartas; i++)
    {
        if (strcasecmp(nomeCompara, novaCarta[i].nome) == 0)
        {
    ...
    do
    {
        printf("Super Trunfo (0 - Não, 1 - Sim): ");
        scanf("%d", &varBool);
        setbuf(stdin, NULL);
    } while (varBool < 0 || varBool > 1); // do while que aceita 0 ou 1

    // Conversão de inteiro para booleano
    novaCarta->super_trunfo = varBool ? true : false;
...
// Função para carregar as cartas do arquivo CSV
void carregarCartas(Cartas **cartas, int *quantidadeCartas)
{
    printf("carregar Cartas\n");
